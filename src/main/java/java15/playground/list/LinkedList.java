...
public sealed interface LinkedList<T>permits LinkedList.Nil, LinkedList.Cons {
    ...
    static <T> LinkedList<T> init(LinkedList<T> list) {
        return execute(
                ...
                cons -> execute(
                        cons.next,
                        nil -> new Nil<>(),
                        consInner -> new Cons<>(cons.value, init(cons.next)))
        );
    }
    ...
    static <T> LinkedList<T> append(LinkedList<T> list1, LinkedList<T> list2) {
        return execute(
                list1,
                nil -> list2,
                cons -> new Cons<>(cons.value, append(cons.next, list2))
        );
    }
    ...
    private static <T> int length(LinkedList<T> list, int counter) {
        return execute(
                list,
                nil -> counter,
                cons -> length(cons.next, counter + 1)
        );
    }

    private static <T, R> R execute(
            LinkedList<T> list,
            Function<Nil<T>, R> nilHandler,
            Function<Cons<T>, R> consHandler
    ) {
        if (list instanceof Nil<T> nil) {
            return nilHandler.apply(nil);
        } else if (list instanceof Cons<T> cons) {
            return consHandler.apply(cons);
        } else {
